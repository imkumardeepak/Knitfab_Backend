            ...
            try
            {
                ...
                if (associatedLots.Any())
                {
                    ...
                    foreach (var lot in associatedLots)
                    {
                        ...
                        lot.VoucherNumber = salesOrderWeb.VoucherNumber;
                        lot.PartyName = salesOrderWeb.BuyerName;
                        lot.OtherReference = salesOrderWeb.OtherReference;

                        // Find corresponding item in the updated Sales Order to update item level fields
                        var matchingItem = salesOrderWeb.Items.FirstOrDefault(item => item.Id == lot.SalesOrderItemId);
